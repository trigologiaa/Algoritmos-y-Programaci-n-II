...
// ShowInOrder retorna una lista de elementos del árbol en orden inorder.
// La responsabilidad la delega en el nodo raíz, que sabe obtener su
// recorrido en inorder.
func (t *BinaryTree[T]) ShowInOrder() []T {
	result := make([]T, 0, t.Size())
	if t.root != nil {
		t.root.showInOrder(&result)
	}
	return result
}

// showInOrder es un método recursivo que recorre el árbol en inorder
// y actualiza una lista de elementos.
func (n *BinaryNode[T]) showInOrder(s *[]T) {
	if n.left != nil {
		n.left.showInOrder(s)
	}
	*s = append(*s, n.data)
	if n.right != nil {
		n.right.showInOrder(s)
	}
}